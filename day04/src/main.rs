...
fn words(row: i32, col: i32, len: i32) -> Vec<Vec<(i32, i32)>> {
    (-1..=1)
        .flat_map(move |r| (-1..=1).map(move |c| word(row, col, r, c, len)))
        .collect()
}

fn all_on_board(rows: &i32, cols: &i32, word: &Vec<(i32, i32)>) -> bool {
    word.iter()
        .all(|(r, c)| r >= &0 && c >= &0 && r < rows && c < cols)
}

fn spells(coords: &Vec<(i32, i32)>, letters: &Vec<Vec<char>>, word: &str) -> bool {
    let found: String = coords
        .iter()
        .map(|(r, c)| letters[*r as usize][*c as usize])
        .collect();
    found == word
}
...
const ACCEPTABLE_XMAS: [&'static str; 4] = ["MMASS", "SSAMM", "MSAMS", "SMASM"];
...
fn main() {
    ...
    let coords: Vec<(i32, i32)> = (0..rows)
        .flat_map(move |r| (0..cols).map(move |c| (r, c)))
        .collect();

    let part1 = coords
        .iter()
        .flat_map(|(r, c)| words(*r, *c, 4))
